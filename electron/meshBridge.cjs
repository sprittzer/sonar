...
function startMeshBridge(opts = {}) {
  const WS_PORT = Number(opts.wsPort || process.env.BRIDGE_PORT || 8788)
  const UDP_PORT = Number(opts.udpPort || process.env.MESH_UDP_PORT || 41234)
  ...
  function upsertPeer(peer) {
    ...
    peers.set(peer.nodeId, { ...peer, lastSeenMs: Date.now() })
    emitPeers()
  }
  ...
  function buildHello(kind) {
    return { kind, nodeId, displayName, port: UDP_PORT, capabilities }
  }
  ...
  function onMeshEnvelope(envelope, sourceKey = '') {
    ...
    if (forMe) emit('meshPacket', { envelope })

    const ttl = Number(envelope.ttl || 0)
    if (ttl <= 0) return
    if (to !== nodeId) forwardEnvelope({ ...envelope, ttl: ttl - 1 }, sourceKey)
  }
  ...
  function onUdpMessage(raw, rinfo) {
    ...
    if (data.kind === 'HELLO' || data.kind === 'HELLO_ACK') {
      ...
      if (data.kind === 'HELLO') sendHelloAckTo(rinfo.address, data.port || rinfo.port)
      return
    }

    if (data.kind === 'MESH' && data.envelope) {
      onMeshEnvelope(data.envelope, `${rinfo.address}:${rinfo.port}`)
    }
  }
  ...
  async function startMdns() {
    ...
    service = bonjour.publish({
      ...
      protocol: 'udp',
      port: UDP_PORT,
      txt: { nodeId, displayName, caps: capabilities.join(',') }
    })
  ...
  function startMesh(params = {}) {
    ...
    socket.on('error', (error) => emit('error', { message: `UDP error: ${error.message}` }))

    helloTimer = setInterval(broadcastHello, HELLO_INTERVAL_MS)
    pruneTimer = setInterval(prunePeers, 3000)

    startMdns().catch((error) => warn(`mDNS helper init failed: ${error.message}`))
  }
  ...
  wss.on('connection', (ws) => {
    ...
    ws.on('message', (raw) => {
      ...
      if (msg.action === 'start') {
        try { startMesh(msg.payload || {}) } catch (error) { emit('error', { message: `Start failed: ${error.message}` }) }
        return
      }
      if (msg.action === 'stop') {
        stopMesh()
      ...
      if (msg.action === 'sendPacket' && msg.payload?.envelope) {
        onMeshEnvelope(normalizeEnvelope(msg.payload.envelope))
        return
      }
  ...
  wss.on('listening', () => {
    console.log(`[bridge] Mesh bridge listening on ws://0.0.0.0:${WS_PORT}`)
  })
  wss.on('error', (error) => {
...
module.exports = { startMeshBridge }
